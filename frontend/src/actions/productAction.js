...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      if (category) {
        link = `${proxy}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}
...
export const getAdminProduct = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${proxy}/api/v1/admin/products`);
...
export const getProductDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${proxy}/api/v1/product/${id}`);
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`${proxy}/api/v1/admin/product/${id}`);
    dispatch({
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${proxy}/api/v1/reviews?id=${id}`);
...
export const deleteReviews = (reviewId, productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `${proxy}/api/v1/reviews?id=${reviewId}&productId=${productId}`
    );
